...
# Global variable for delay mode
SLOW_MODE = False
DELAY_SECONDS = 2  # Delay in seconds between operations in slow mode
...
# Function to apply delay if slow mode is enabled
def apply_delay_if_needed():
    if SLOW_MODE:
    ...
    def generate_addresses_thread():
        ...
        while True:
            ...
            for address in addresses:
                ...
                # Apply delay if in slow mode (after each address check)
                apply_delay_if_needed()
            
            # Additional delay after processing all addresses
            apply_delay_if_needed()
...
# Function to get user choice for speed mode
def get_speed_mode():
    global SLOW_MODE, DELAY_SECONDS
    
    print("\nSelect speed mode:")
    print("1. Normal Mode - Maximum speed (higher CPU/Power usage)")
    print("2. Slow Mode - Reduced speed (lower CPU/Power usage)")
    
    while True:
        speed_choice = input("\nEnter your choice (1 or 2): ").strip()
        if speed_choice == '1':
            SLOW_MODE = False
            print("\n✅ Normal mode selected - Running at maximum speed")
            break
        elif speed_choice == '2':
            SLOW_MODE = True
            # Ask for custom delay
            print("\nHow many seconds delay between operations?")
            print("(Recommended: 1-5 seconds for significant power reduction)")
            delay_input = input("Enter delay in seconds (default 2): ").strip()
            if delay_input:
                try:
                    DELAY_SECONDS = float(delay_input)
                    print(f"✅ Slow mode selected - {DELAY_SECONDS} second(s) delay between operations")
                except ValueError:
                    DELAY_SECONDS = 2
                    print("✅ Slow mode selected - Using default 2 second delay")
            else:
                DELAY_SECONDS = 2
                print("✅ Slow mode selected - Using default 2 second delay")
            break
...
def main():
    ...
    # First, ask for the operation mode
    print("\nSelect operation mode:")
    print("1. Online - Check balances of all addresses (uses internet)")
    print("2. Offline - Match addresses with config addresses only (no internet)")
    ...
    # Then, ask for speed mode
    get_speed_mode()
    
    # Then ask for number of threads
    while True:
        try:
    ...
    print(f"⚡ Speed Mode: {'SLOW' if SLOW_MODE else 'NORMAL'}")
    if SLOW_MODE:
        print(f"⏱️  Delay: {DELAY_SECONDS} seconds between operations")
    print("=" * 50)
